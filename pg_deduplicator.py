...
try:
    conn=psycopg2.connect("dbname='sagaie' user='postgres'")
except:
...
SELECT tc.table_schema, tc.table_name, kc.position_in_unique_constraint, array_to_string(array_agg(kc.column_name::text), ', ') as column_name
...
WHERE (tc.constraint_type = 'PRIMARY KEY'
  ...
  GROUP BY  tc.table_schema, tc.table_name, kc.position_in_unique_constraint
...
SELECT FORMAT ('SELECT %I, count(%I) as n_duplicate_rows, array_agg(%I::text) as pks_uks FROM %I.%I GROUP BY %I HAVING count(%I) >1;', 
